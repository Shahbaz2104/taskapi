...
const registerAndLogin = async (username = "sessuser") => {
  await request(app)
...
describe("Sessions / devices", () => {
  ...
  it("lists active sessions with metadata", async () => {
    ...
    // A second device logs in (registration itself also created one)
    await request(app)
      .post("/api/v1/auth/login")
    ...
    // Newest first — the User-Agent login is sessions[0]
  ...
  it("revokes a single session by id — its refresh token dies, others survive", async () => {
    // Registration creates session 0; this login is session 1
    ...
    // The surviving sessions (register + first login) are still active
    ...
    // And the first login's refresh token still rotates
    ...
    // (reuse detection then revokes all remaining sessions, by design)
